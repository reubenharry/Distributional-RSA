...
\section{Overview of metaphor}
	...
	% For a given metaphor, only certain properties of the target are described by the source.
	% , and which these are depend on the metaphor and the context. 
	% For instance, (\ref{dog}), said of a sleeping dog, could convey that it is unresponsive, but said of a large alert dog, could convey that it is heavy.
	...
	% Examples like (\ref{time}) and (\ref{basement}), and more generally the ability to interpret novel metaphors, suggest that metaphors cannot be explained merely as fixed conventional phrase meanings. The benefit of the pragmatic view of metaphor is the ability to explain the interpretation of novel metaphors in a way which takes into account an underlying semantics (e.g. the conventional meanings of \emph{dog} and \emph{rock}).

	% While certain metaphors are conventional - comparing someone to a lion tends to connote bravery - examples like (\ref{dog}) suggest that novel interpretations . 
...
\section{A Bayesian model of metaphor interpretation} \label{rsa}
	...
	\paragraph{Projections} 
		...
		capturing the degree to which each $w$ extends along $v$, ignoring orthogonal dimensions. Geometrically, this amounts to dropping a line from an input vector $w$ at a right angle onto $v$, as depicted in figure \ref{fig:1}. These projections exploit the linear structure of the word embedding space \cite{pennington2014glove}, though see \cite{linzen2016issues,finley2017analogies} for potential caveats.
