...
\author[a,c,1]{Reuben Cohn-Gordon}
\author[b,1,2]{Leon Bergen} 
...
Humans easily interpret previously unseen metaphorical language, by TODO etc

By incorporating a computationally BLAH TODO

we construct a system capable of providing interpretations to unseen metaphors. We show that this system outperforms a comparable baseline which uses similar word-level information, but no explicit reasoning TODO
...
\section{A Bayesian model of metaphor interpretation} \label{rsa}
	...
	The most basic version of RSA \cite{frank2012predicting} is incapable of interpreting metaphors, due to the strict assumption that the speaker's utterances are literally true. To address this, Kao et al. \cite{kao} propose a model $\QLONE$, defined in (\ref{defl1}). 
...
% \subsubsection{L\textsubscript{0} Inference}
	...
	Intuitively, the vectorial interpretation of $L_0$ amounts to the process shown in figure \ref{fig:2d}, where a ball, corresponding to the prior, is moved in the direction of the point corresponding to the received utterance. To calculate $L_0$ analytically, we make use of Gaussian conjugacy. When the prior $P_L$ is defined as in Equation \ref{vect:prior}, and the semantic interpretation is defined as in Equation \ref{vect:sem}, then conjugacy implies that the listener posterior is given by:
	 ...
	 \begin{examples}
	\item $L_0(w|u) = P_{\mathcal{N}}(w\vert\mu=\frac{\sigma_1^2 \sigma_2^2}{\sigma_1^2+\sigma_2^2}\cdot(\frac{{\textrm{\emph{E(target)}}}}{\sigma_1^2}+\frac{{\textrm{\emph{E(source)}}}}{\sigma_2^2}), \sigma=\frac{\sigma_1^2 \sigma_2^2}{\sigma_1^2+\sigma_2^2})$  \label{eq:L0-inference}
	 \end{examples}
...
\section{Experimental Evaluation} \label{exp}
	...
	\subsection{Experimental Design}
		...
		\end{subfigure}\begin{subfigure}{0.5\textwidth}
			...
			\vspace*{-1.5cm}\hspace{6em}\includegraphics[width=19.1cm,height=6.5cm,angle=270]{images/resultsbarplot.png}
		\end{subfigure}
			\caption{The 109 metaphors used in the experiment, and baseline and $\protect\QLONE$ interpretations. Bar positions right of center indicate a preference for the pragmatic model. TODO: add more here
