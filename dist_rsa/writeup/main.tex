...
\section{Bayesian pragmatics with a distributional semantics} \label{bayesdist}
	...
	\subsubsection{The listener's prior} 
		...
		\begin{exe}
		\ex $P_L(w) = P_{\mathcal{N}}(w\vert\mu=E(\textrm{\emph{source}}),\sigma=\sigma_1)$ \label{vect:prior}
		% \overrightarrow{\mathit{source}}
		\end{exe}

		We can view $P_L$ as representing uncertainty over the position of the entity or concept that the source noun represents.
	...
	\subsubsection{The semantics}
		...
		\begin{exe}
		\vspace{-1em}
		\ex $\llbracket u\rrbracket(w) = P_{\mathcal{N}}(w\vert\mu=E(\textrm{\emph{target}}),\sigma=\sigma_2)$ \label{vect:sem}
	...
	\subsubsection{Projections} 
		...
		\centering
			...
			\caption{In this 2D example, the vectors representing \emph{Jane} and \emph{John} are mapped onto $\protect\overrightarrow{endurance}$ and $\protect\overrightarrow{ruthlessness}$.}
...
\subsubsection{L\textsubscript{0} Inference}
	...
	$L_0(w|u) = P_{\mathcal{N}}(w\vert\mu=\frac{\sigma_1^2 \sigma_2^2}{\sigma_1^2+\sigma_2^2}\cdot(\frac{{\textrm{\emph{source}}}}{\sigma_1^2}+\frac{{\textrm{\emph{target}}}}{\sigma_2^2}), \sigma=\frac{\sigma_1^2 \sigma_2^2}{\sigma_1^2+\sigma_2^2})$
