...
def l1_cat_2d_exp(metaphor):
    ...
    print("RESULTS\n")
    for x,y in results[:20]:
        print("QUD:",x,np.exp(y))
        print("world sample under qud - subj:",distance_under_projection(real_vecs[subj],np.squeeze(run.world_samples),x,real_vecs))
        print("world sample under qud - pred:", distance_under_projection(real_vecs[pred],np.squeeze(run.world_samples),x,real_vecs))
...
if __name__ == "__main__":
    ...
    l1_cat_2d_exp(("poison","love"))
    l1_cat_2d_exp(("man","tree"))
