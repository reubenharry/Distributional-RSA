...
# np_rsa(s1_world,qud_mat,vecs,vec_length,sig1,sig2, frequencies, listener_prior_mean, possible_utterances, utterance, lam=0,debug=True)
	...
	def l0(u,listener_prior_mean,sig1,sig2):
		n = 1
		y = u
		mu1 = listener_prior_mean
		sigma1 = sig1 ** 2
		sigma2 = sig2 ** 2
	...
	def score_s1_utterance(world,qud,utterance,sig1,sig2):
		...
		# unprojected_score = scipy.stats.multivariate_normal(listener_mu,listener_sigma).logpdf(world)
		# log_score = unprojected_score

		# print("unprojected log likelihood:",unprojected_score)
	...
	def s1(s1_world,qud_mat,frequencies,possible_utterances,sig1,sig2):

		# print(qud_mat.shape,s1_world.shape,vecs['predator'].shape)

		# s1_world = np.mean(s1_world.reshape(1,vecs[world].shape[0]) + (lam*qud_mat.T))

		# for i in range(len(qud_mat.T)):
		# 	s1_world = s1_world+lam*qud_mat.T[i] #shift the s1_world by the qud_vec
		...
		# return score_s1_utterance(s1_world,qud_mat,vecs['human'],s1,s2,world_mean) 

		norm = scipy.misc.logsumexp([x[1] + (score_s1_utterance(s1_world,qud_mat,x[0],sig1,sig2)) for x in uniform_draw])
		# return norm
		out = [(x[1] + (score_s1_utterance(s1_world,qud_mat,x[0],sig1,sig2))) - norm for x in uniform_draw]
	...
	out = s1(s1_world=s1_world,qud_mat=qud_mat,frequencies=frequencies,possible_utterances=possible_utterances,sig1=sig1,sig2=sig2)
