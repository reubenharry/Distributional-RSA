...
def l1_cat_2d_exp(metaphor):
    ...
    # break
    quds = ["unyielding","wooden","strong","big","connected","leafy","abstract"]
    possible_utterance_adjs = quds
    print("QUDS",quds[:50]) 
    # possible_utterances = possible_utterance_nouns[100:]
    possible_utterances = ['tree','shrub','branch']
    # +possible_utterance_adjs[:40]
    ...
    if (subj not in vecs) or (pred not in vecs):
        raise Exception(subj+pred)
...
if __name__ == "__main__":
    ...
    l1_cat_2d_exp(("man","shrub"))
    l1_cat_2d_exp(("man","stump"))
